 ...
 *
 * @property \CakeSilverCms\Model\Table\LanguagesTable $Languages
 *
 * @method \CakeSilverCms\Model\Entity\Language[]|\Cake\Datasource\ResultSetInterface paginate($object = null, array $settings = [])
 */
class LanguagesController extends AppController
{
    ...
    public function add()
    {
        ...
        if ($this->request->is('post')) {
            $is_default = $this->request->getData('is_default');
            if ($is_default) {
    ...
    public function edit($id = null)
    {
        ...
        if ($this->request->is(['patch', 'post', 'put'])) {
            ...
            $language->modified_at = date('Y-m-d H:i:s');
            $language              = $this->Languages->patchEntity($language, $this->request->getData());
            if ($this->Languages->save($language)) {
                $this->Flash->success(__('The language has been saved.'));
